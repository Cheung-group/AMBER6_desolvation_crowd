...
C Margaret:Langevin
#include "LAN.h"
C  March 21,2014   Modified by Jianfa Chen
C      DOUBLE PRECISION NOISE(200) 
      DOUBLE PRECISION NOISE(2000) 
c
...
cc water=0.1439 kcal*ps/mol*AA**3 * 20.445 
cc If NR3>200 need to reset the array of NOISE

cc March 21, 2014   Modified by Jianfa Chen
c	if (NR3.gt.200) then
	if (NR3.gt.2000) then
	open(76,file='alert')
	write(76,*) 'reset size of NOISE NR3= ',NR3
	end if 
 
c        CALL DLARNV(3,JSEED,180,NOISE)   
        CALL DLARNV(3,JSEED,1800,NOISE)   
cc end modified by Jianfa

cc   do not fix the first atom	
cc origin   DO I3  = 1,NR3+iscale
cc        DO I3  = 4,NR3+iscale
        DO I3  = 1,NR3+iscale

	F(I3) = F(I3)*dtx/eta
        tmpp= 2*boltz*temp0*dtx

c        write(75,*)JSEED(1),JSEED(2),JSEED(3),JSEED(4)
c        CALL DLARNV(3,JSEED,1,NOISE)   
c        CALL DLARUV(JSEED,1,NOISE)   

	gammaa = NOISE(I3)*sqrt(tmpp/eta)

	
        x(I3)=x(I3)+F(I3)+gammaa 
c        write(75,*)I3,x(i3),F(I3),gammaa
